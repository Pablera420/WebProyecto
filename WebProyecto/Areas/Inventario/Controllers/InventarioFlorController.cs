...
namespace WebProyecto.Areas.Inventario.Controllers
{
    ...
    public class InventarioFlorController : Controller
    {
        ...
        public InventarioFlorController(ApplicationDbContext db, IUnidadTrabajo unidadTrabajo)
        {
            _db = db;
        ...
        public IActionResult NuevoInventario(int? inventarioId)
        {
            ...
            inventarioVM.FlorLista = _db.Flores.ToList().Select(p =>
                                new SelectListItem
                                {
                                    Text = p.Descripcion,
                                    Value = p.Id.ToString()
            ...
            if (inventarioId != null)
            {
                ...
                inventarioVM.InventarioDetalles =
                    _db.InventarioDetalle.Include(p => p.Flor).Include(m =>
                             m.Flor.Especie).
        ...
        [HttpPost]
        public IActionResult AgregarProductoPost(int productoflor, int cantidad, int inventarioId)
        {
            ...
            var bodegaProducto = _db.BodegaProductos.Include(b =>
                                b.Flor).FirstOrDefault(b => b.FlorId == productoflor &&
            ...
            var detalle = _db.InventarioDetalle.Include(p =>
                           p.Flor).FirstOrDefault(d => d.FlorId == productoflor &&
            ...
            if (detalle == null)
            {
                ...
                inventarioVM.InventarioDetalle.FlorId = productoflor;
        ...
        public IActionResult GenerarStock(int Id)
        {
            ...
            {
                var bodegaProducto = _db.BodegaProductos.Include(p
                           => p.Flor).FirstOrDefault(b
                           => b.FlorId == item.FlorId
                ...
                else
                {
                    ...
                    bodegaProducto.FlorId = item.FlorId;
                    bodegaProducto.Cantidad = item.Cantidad;
                    _db.BodegaProductos.Add(bodegaProducto);
                }
            }
        ...
        public IActionResult DetalleHistorial(int id)
        {
            var detalleHistorial =
                _db.InventarioDetalle.Include(p => p.Flor)
                .Include(m => m.Flor.Especie).Where(d => d.InventarioId == id);
        ...
        public object ImprimirInventario(string info)
        {
            ...
            if (info == null)
            {
                inventarioVM.InventarioDetalles =
                  _db.InventarioDetalle.Include(p => p.Flor)
                  .Include(m => m.Flor.Especie)
                  .Include(c => c.Flor.CategoriaFlor)
                  .OrderBy(c => c.Flor.CategoriaFlor.Nombre).ToList();
            }
            else
            {
                inventarioVM.InventarioDetalles =
               _db.InventarioDetalle.Include(p => p.Flor).Include(m =>
                m.Flor.Especie).Include(c => c.Flor.CategoriaFlor)
                .Where(p => p.Flor.Descripcion.Contains(info))
               .OrderBy(c => c.Flor.CategoriaFlor.Nombre).ToList();
                if (inventarioVM.InventarioDetalles == null)
                {
                   ...
                   _db.InventarioDetalle.Include(p => p.Flor).Include(m =>
                    m.Flor.Especie).Include(c => c.Flor.CategoriaFlor)
                    .Where(p => p.Flor.CategoriaFlor.Nombre.Contains(info))
                   .OrderBy(c => c.Flor.CategoriaFlor.Nombre).ToList();
                }
            }
        ...
        public IActionResult ObtenerTodos()
        {
            var ListaBodegaProductos = _db.BodegaProductos.Include(b =>
                        b.Bodega).Include(p => p.Flor).ToList();
